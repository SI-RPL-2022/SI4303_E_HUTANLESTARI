...
@section('content')
    ...
    <div class="container">
        <h2 class="text-center text-success my-5">
            Form Informasi flora
        </h2>

        <div class="card rounded shadow-sm bg-card">
            <div class="card-body p-5">
                <form action="{{route('admin.informasifloraeditpost' , ['id'=>$data->id])}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <div class="form-group mb-3">
                        <label for="nama">Name Flora</label>
                        <input type="text" name="nama_flora" class="form-control" value="{{$data->nama_flora}}">
                    </div>
